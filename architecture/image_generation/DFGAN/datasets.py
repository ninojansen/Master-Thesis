...
class EasyVQADataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.easyVQA_test = EasyVQADataset(self.data_dir, "test",)
        if stage == 'fit' or stage is None:
            self.easyVQA_train = EasyVQADataset(self.data_dir, "train")

    def train_dataloader(self):
        return DataLoader(self.easyVQA_train, batch_size=self.batch_size, drop_last=True,
                          shuffle=True, num_workers=self.num_workers, pin_memory=True)

    def val_dataloader(self):
        return DataLoader(self.easyVQA_test, batch_size=self.batch_size, drop_last=True,
                          num_workers=self.num_workers, pin_memory=True)
...
class EasyVQADataset(data.Dataset):
    ...
    def load_image(self, id):
        img_path = os.path.join(self.im_dir, f"{id}.png")
        img = Image.open(img_path).convert('RGB')
