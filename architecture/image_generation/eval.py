...
def parse_args():
    ...
    parser.add_argument('--ckpt', dest='ckpt', type=str,
                        default="/home/nino/Dropbox/Documents/Master/Thesis/architecture/image_generation/output")
...
if __name__ == "__main__":
  ...
  #  sample_questions = random.sample(test_questions, n_questions)
    ...
    im_dir = os.path.join(args.output_dir, "images")
    os.makedirs(im_dir, exist_ok=True)

    for images, (ckpt, ef_type, name) in zip(sample_images, model_ckpts):
        torchvision.utils.save_image(
            images, os.path.join(im_dir, f"{ef_type}_{name}.png"),
            normalize=True, nrow=n_questions, padding=16, pad_value=255)
