...
class GANTrainer(object):
    ...
    def train_Dnet(self, idx, count):
        ...
        # log
        if flag == 0:
            summary_D = summary.scalar('D_loss%d' % idx, errD)
    ...
    def train_Gnet(self, count):
        ...
        for i in range(self.num_Ds):
            ...
            if flag == 0:
                summary_G = summary.scalar('G_loss%d' % i, errG)
        ...
        if cfg.TRAIN.COEFF.COLOR_LOSS > 0:
            ...
            if flag == 0:
                sum_mu = summary.scalar('G_like_mu2', like_mu2)
                self.summary_writer.add_summary(sum_mu, count)
                sum_cov = summary.scalar('G_like_cov2', like_cov2)
                self.summary_writer.add_summary(sum_cov, count)
                if self.num_Ds > 2:
                    sum_mu = summary.scalar('G_like_mu1', like_mu1)
                    self.summary_writer.add_summary(sum_mu, count)
                    sum_cov = summary.scalar('G_like_cov1', like_cov1)
                    self.summary_writer.add_summary(sum_cov, count)
...
class condGANTrainer(object):
    ...
    def train_Gnet(self, count):
        ...
        for i in range(self.num_Ds):
            ...
            errG_total = errG_total + errG
            if flag == 0:
                summary_D = summary.scalar('G_loss%d' % i, errG)
                self.summary_writer.add_summary(summary_D, count)
    ...
    def evaluate(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            netG.eval()
            for ii in range(10):
                save_dir = '%s/iteration%d/%s' % (s_tmp, iteration,ii)
                for step, data in enumerate(self.data_loader, 0):
